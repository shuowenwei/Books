function [total_cost] = iterative_dfs(map,initial,dstn_city)
...
for j=1:30
    ...
    path_table(j,1:length(path))=path;
    path_length(j)=length(path);
end
shortest=min(path_length);
position=find(path_length==shortest);
position=position(1);
path=[];
path=path_table(position,1:shortest);
